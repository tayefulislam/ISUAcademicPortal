...
export const TEMPLATES = {
  FILE_UPLOADED: {
    ...
    message: (v) => `${v.actorName} uploaded "${v.fileName}"${v.courseName ? ` in ${v.courseName}` : ''}`,
    url: (v) => `/student/materials/${v.fileId}`,
  },
  FILE_UPDATED: {
    title: () => 'Course Material Updated',
    message: (v) => `"${v.fileName}" was updated${v.courseName ? ` in ${v.courseName}` : ''}`,
    url: (v) => `/student/materials/${v.fileId}`,
  },
  COURSE_MATERIAL: {
    title: () => 'New Course Material',
    message: (v) => `${v.actorName} uploaded "${v.fileName}"${v.courseName ? ` — ${v.courseName}` : ''}`,
    url: (v) => `/student/materials/${v.fileId}`,
  },
  ASSIGNMENT_CREATED: {
    title: () => 'New Assignment',
    message: (v) => `${v.title} has been published${v.deadline ? `. Due ${v.deadline}` : ''}`,
  ...
  ASSIGNMENT_RESULT: {
    ...
    message: (v) => `Your result for "${v.title}" is now available (${v.marks}/${v.maxMarks})`,
    url: (v) => `/student/assignments/${v.assignmentId}`,
  },
  EXAM_CREATED: {
    title: () => 'New Exam',
    message: (v) => `${v.title} has been published${v.startAt ? `. Starts ${v.startAt}` : ''}`,
