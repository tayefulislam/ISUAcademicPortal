...
describe('emit', () => {
  test('creates one Notification per resolved recipient with the rendered template', async () => {
    const result = await emit({
      ...
      vars: { actorName: 'Prof. Rahim', fileName: 'Lecture 5.pdf', courseName: 'DB Systems', fileId: 'f1' },
    ...
    assert.equal(docs[0].url, '/student/materials/f1');
    assert.equal(docs[0].isRead, false);
  });

  test('returns { created: 0 } and writes nothing for an empty recipient list', async () => {
    const result = await emit({ type: 'SYSTEM', vars: { title: 't', message: 'm' }, recipients: [] });
  ...
  test('never notifies the actor themself, even if they appear in the recipient list', async () => {
    ...
    const recipients = (await Notification.find({})).map((n) => String(n.recipient));
  ...
  describe('idempotency (spec: a retried request must not double-notify)', () => {
    test('emitting the exact same event twice creates the row only once per recipient', async () => {
      ...
      assert.equal(await Notification.countDocuments({ recipient: userA._id, type: 'ASSIGNMENT_RESULT' }), 1);
    });

    test('a genuinely different event (different type) for the same entity is NOT deduped', async () => {
      const entityId = new mongoose.Types.ObjectId();
      await emit({ type: 'EXAM_CREATED', entityType: 'QUIZ', entityId, vars: { title: 'Midterm', quizId: 'q1' }, recipients: [userA._id] });
      await emit({ type: 'EXAM_UPDATED', entityType: 'QUIZ', entityId, vars: { title: 'Midterm', quizId: 'q1' }, recipients: [userA._id] });
      assert.equal(await Notification.countDocuments({ recipient: userA._id }), 2);
    });
    ...
    test('two different admin broadcasts (both SYSTEM, no natural entity) never collide with each other', async () => {
      ...
      await emit({ type: 'SYSTEM', vars: { title: 'First', message: 'm1' }, recipients: [userA._id] });
      await emit({ type: 'SYSTEM', vars: { title: 'Second', message: 'm2' }, recipients: [userA._id] });
      assert.equal(await Notification.countDocuments({ recipient: userA._id, type: 'SYSTEM' }), 2);
    });
  });
  ...
  describe('per-type notification preferences', () => {
    test('a user who disabled a type receives no Notification row for it, but still gets other types', async () => {
      ...
      await emit({
        ...
        entityType: 'FILE',
        entityId: new mongoose.Types.ObjectId(),
        vars: { actorName: 'X', fileName: 'f.pdf', courseName: 'C', fileId: 'f1' },
        recipients: [userA._id, userB._id],
      });
      const courseMaterialRecipients = (await Notification.find({ type: 'COURSE_MATERIAL' })).map((n) => String(n.recipient));
      ...
      assert.equal(await Notification.countDocuments({ type: 'NOTICE_CREATED', recipient: userA._id }), 1);
    });
    ...
    test('SYSTEM notifications are mandatory and ignore per-type preferences entirely', async () => {
      ...
      await userA.save();
      await emit({ type: 'SYSTEM', vars: { title: 'Down', message: 'now' }, recipients: [userA._id] });
      assert.equal(await Notification.countDocuments({ type: 'SYSTEM', recipient: userA._id }), 1);
    });
    ...
    test('a user missing notificationPreferences.types altogether (pre-existing account) defaults to enabled', async () => {
      ...
      await User.collection.updateOne({ _id: userC._id }, { $unset: { notificationPreferences: '' } });
      await emit({
        type: 'EXAM_REMINDER',
        entityType: 'QUIZ',
        entityId: new mongoose.Types.ObjectId(),
      ...
      assert.equal(await Notification.countDocuments({ type: 'EXAM_REMINDER', recipient: userC._id }), 1);
    });
  });
});
