...
function ListSettingCard({ title, description, items, onSave }) {
  ...
  return (
    <div className="max-w-xl bg-white border border-slate-200 rounded-xl p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {items.length === 0 && <span className="text-xs text-slate-400">No domains configured — official-email auto-approval is off.</span>}
        {items.map((item) => (
...
export default function SuperAdminSettings() {
  ...
  const saveList = async (key, value, label) => {
    try {
        ...
        <div className="space-y-6 mb-10">
          {listSettings.map((l) => (
            <ListSettingCard
              ...
              items={values[l.key] || []}
              onSave={(v) => saveList(l.key, v, l.label)}
            />
          ))}
        </div>
      )}
