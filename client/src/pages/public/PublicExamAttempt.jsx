...
// Mirrors client/src/pages/QuizAttempt.jsx exactly in behavior — same
// shared AnswerInput/useCountdown, same autosave/refresh-resume/auto-submit
// flow — the only difference is identity: a guestToken (localStorage) or a
// logged-in participant's normal auth token, instead of always requiring
// the latter.
export default function PublicExamAttempt() {
  ...
  useEffect(() => {
    publicExamApi
      .getAttempt(slug, attemptId, guestToken)
      .then((res) => {
  ...
  const doSubmit = useCallback(
    async (silent) => {
      setSubmitting(true);
      try {
        await publicExamApi.submitAttempt(slug, attemptId, guestToken);
        if (!silent) toast('Exam submitted', 'success');
        navigate(`/exam/${slug}/result/${attemptId}`, { replace: true });
      } catch (err) {
  ...
  const saveAnswer = (questionId, patch) => {
    ...
    saveTimers.current[questionId] = setTimeout(async () => {
      try {
        await publicExamApi.saveAnswer(slug, attemptId, guestToken, { questionId, ...answers[questionId], ...patch });
      } catch (err) {
        if (err.response?.data?.autoSubmitted) {
          toast('Time is up — your exam was auto-submitted', 'error');
          navigate(`/exam/${slug}/result/${attemptId}`, { replace: true });
        }
      }
    }, 500);
  ...
  if (loading || !attempt) {
    return <div className="max-w-5xl mx-auto px-4 py-16 text-center text-slate-400">Loading exam...</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4 sticky top-0 bg-slate-50 py-2 z-10">
        <div>
          <h1 className="text-lg font-bold text-slate-800">Exam in progress</h1>
