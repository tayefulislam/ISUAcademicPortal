...
The client is installable and works offline for previously-visited content,
via `vite-plugin-pwa` (configured in `client/vite.config.js`).

- **Manifest**: name, icons (192/512 + maskable variants under
  `client/public/icons/`), `display: standalone`, theme color matching the
  brand palette. Regenerate icons from the SVG sources with
  `npm run gen-icons --prefix client` (uses `sharp`; edit
  `client/scripts/icon-source.svg` / `icon-maskable-source.svg` first if you
  want different branding).
- **Service worker**: generated by Workbox at build time
  (`registerType: 'autoUpdate'`). It precaches the app shell (JS/CSS/HTML/
  icons/manifest) and applies runtime caching:
  - API GET requests (departments/courses/files/search/etc.) —
    `StaleWhileRevalidate`, so the UI shows the last-known data instantly
    and refreshes in the background.
  - Files served from the backend's `/uploads/*` route, plus ImgBB/Uploadcare
    CDN images — `CacheFirst`, so previously-viewed files/images open
    instantly and work offline.
  - Mutating requests (POST/PUT/PATCH/DELETE) are never cached — Workbox
    only intercepts `GET` by default.
- **Update flow**: `client/src/components/PwaUpdatePrompt.jsx` (mounted in
  `App.jsx`) shows a toast + a persistent "Update available" button when a
  new deployment is live, and re-checks whenever the tab regains focus —
  no silent auto-reload that could interrupt an in-progress upload.
- The service worker only builds in production (`npm run build` /
  `vite preview`) — `npm run dev` does not register one, so local
  development always sees fresh code with no cache to bust.

To verify installability: build (`npm run build --prefix client`), serve the
`dist/` output over HTTP (`npm run preview --prefix client`), open it in a
real browser (not this sandbox — service workers are blocked in this
Claude Code preview pane), and check DevTools → Application → Manifest /
Service Workers, or run a Lighthouse PWA audit.
